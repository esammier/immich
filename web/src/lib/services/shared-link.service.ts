...
export const getSharedLinkActions = ($t: MessageFormatter, sharedLink: SharedLinkResponseDto) => {
  return {
    Edit: {
      ...
      onSelect: () => void handleNavigateEditSharedLink(sharedLink),
    },
    Copy: {
      title: $t('copy_link'),
      icon: mdiContentCopy,
    ...
    ViewQrCode: {
      ...
      icon: mdiQrcode,
      onSelect: () => handleViewSharedLinkQrCode(sharedLink),
    },
    ContextMenu: {
      title: $t('shared_link_options'),
      icon: mdiDotsVertical,
      onSelect: ({ event }) => handleOpenContextMenu(event as MouseEvent, sharedLink),
    },
...
const handleViewSharedLinkQrCode = async (sharedLink: SharedLinkResponseDto) => {
...
export const handleCreateSharedLink = async (dto: SharedLinkCreateDto) => {
  try {
    ...
    void handleViewSharedLinkQrCode(sharedLink);
    return sharedLink;
  } catch (error) {
    const $t = await getFormatter();
...
const handleNavigateEditSharedLink = async (sharedLink: SharedLinkResponseDto) => {
